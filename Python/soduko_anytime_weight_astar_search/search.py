...
class SearchEngine:
    ...
    def _searchOpen(self, goal_fn, heur_fn, fval_function, costbound):
        ...
        while not self.open.empty():
            ...
            ff44 = (4,4) in cur_state.boxes
            ff54 = (5,4) in cur_state.boxes
            if not ff44 and not ff54:
                self.check.append(node.state)
            ...
            """
            # shan
            print('------------ fcost : {} ---------------------'.format(node.state.gval + self.heur_fn(node.state)))
