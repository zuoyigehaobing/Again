...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t *p_out = coremap[frame].pte;
		if(p_out->frame & PG_DIRTY) {
			p_out->swap_off = swap_pageout(frame, p_out->swap_off);
			p_out->frame |= PG_ONSWAP;
			evict_dirty_count++;
		} else {
			evict_clean_count++;
		}
		p_out->frame &= ~PG_VALID;
		p_out->frame &= ~PG_DIRTY;		
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *pt_frm = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	p = pt_frm + PGTBL_INDEX(vaddr);
	...
	if (!(p->frame & PG_VALID)) {
		miss_count++;
		new_page_frame = allocate_frame(p);
		p->frame = ((new_page_frame << PAGE_SHIFT) + (p->frame & PGTBL_MASK));
		if (p->frame & PG_ONSWAP)
			swap_pagein(new_page_frame, p->swap_off);
		else {
			init_frame(new_page_frame, vaddr);
			p->frame |= PG_DIRTY;
		}
	} else
		hit_count++;
