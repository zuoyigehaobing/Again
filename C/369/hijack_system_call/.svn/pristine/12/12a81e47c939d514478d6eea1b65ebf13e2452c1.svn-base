...
void my_exit_group(int status)
{
	/* ============= Functionality ================
	*
	*	- (1) lock my table while editing and get the exiting pid:
	*   - (2) remove that pid from all lists and unlock my table after editing
	*	- (3) call original exit group
	*
	*/
 ...
 */
 asmlinkage long interceptor(struct pt_regs reg) {
 	...
 	/* ============= Functionality ================
 	*
 	*	- (1) if monitored == 0 , don't log any message
 	*   - (2) if monitored == 1, need to check whether monitored
 			  - if the pid is being monitored , log
 			  - else do nothing
 	*	- (3) if monitored == 2,
			  - check the blacklist, if the pid is in the blacklist, do nothing
			  - if the pid not in the black list log the message
 	*	- (4) if monitored have unsafe behaviour , negative or more than 2, report
 	*	- (5) call the original system call
 	*
 	*/
 	...
 	// (3)
 		...
 		// need to check the black list
 	{
 		// if in the black list , don't log
 	...
 	{
 		printk(KERN_DEBUG "interceptor bug @(pid=%d)\n",current->pid);
 	}
 ...
 */
 asmlinkage long my_syscall(int cmd, int syscall, int pid) {
 	...
 	/* ============= Condition checking ================
 	* (1) check the range of the syscall,validation
 	* (2) initialize a variable root for latter root permission checking
 	*/
 	int root;
 	...
 	{
 		/* ============= Condition checking ================
 		* (1) if not root , permission denied
 		* (2) if intercepting a system call that is already intercepted
 		*/
 		if (current_uid() != 0)
 		{
 			return -EPERM;
 		...
 		/* ============= Functionality ================
 		*
 		*   - Make a copy of the origin system call , store in table[syscall].f
 		*   - change the intercepted to 1
 		*	- hijack the sys_call_table
 		*
 		*/
 	...
 	{
 		/* ============= Condition checking ================
 		* (1) if not root , check the permission
 		* (2) can not de-intercept while not being intercepting
 		*/
 		if (current_uid()!=0)
 		{
 			return -EPERM;
 		...
 		/* ============= Functionality ================
 		*
 		*   - restore the origin syscall
 		*	- destroy the monitord list , reinitialize the list head
 		*   - change the intercepted param back to 0
 		*/
 	...
 	else if(cmd == REQUEST_START_MONITORING)
 	{
 		...
 		* (3) if monitoring a pid that is already being monitored
 		*/
 		if(pid < 0)
 			return -EINVAL;
		...
		// if monitored = 2, then add the pid but do nothing
 		...
 		/* ============= Functionality ================
 		*
		*	- check the black list when monitpored == 2
 		*   - handle the case when pid == 0, monitor all pid
 		*	- handle the case when pid !=0 , add that pid to the list
 			  if a pid can not be added to a monitored list, report ENOMEM
 		*/

 		if(table[syscall].monitored == 2)
 		{
 			// check whether in the black list if yes, remove
 		...
 		else if(pid == 0 && root)
 		{
 			spin_lock(&my_table_lock);
 			destroy_list(syscall);
 			table[syscall].monitored = 2; // need to add listcount?
 			INIT_LIST_HEAD (&table[syscall].my_list);
 			spin_unlock(&my_table_lock);
 		}
 	...
 	else if(cmd == REQUEST_STOP_MONITORING)
 	{
 		...
 		* (1) validate pid:
 		* (2) check the permission
 		* (3) Cannot stop monitoring for a pid that is not being monitored
 		* (4) cannot stop monitoring if the syscall has not been intercepted yet
 		*/
 		if(pid < 0)
 			return -EINVAL;
 		if((pid!=0) &&(pid_task(find_vpid(pid),PIDTYPE_PID) == NULL))
 		...
 		/* ============= Functionality ================
 		*
 		*   - handle the case when pid == 0, remove all pid
 		*	- handle the case when pid != 0, remove that pid from list
 		*/
		...
		if(pid == 0 && root)
 		...
 		else
 		{
 			if(table[syscall].monitored == 2)
 			{
 				// add to the block list and return
 				if(add_pid_sysc(pid,syscall) == 0)
 					return 0;
 				return -ENOMEM;
 	...
 	else
 	{
 		/* ============= Error checking ================
 		*
 		*   if the function goes into this block, there's an unsafe
 		*	behaviour while setting monitor's value
 		*/
 		printk(KERN_DEBUG "the command line arguments is invalid\n");
 		return -EINVAL;
 	}
 ...
 */
 static int init_function(void) {
    ...
    *
    *	- initialize struct mytable table[]
    *   - initialize every element in table
    *
    */
    //mytable table[NR_syscalls+1];
	...
	/* ============= Step 2 : Hijack ================
    *
    *	- spin lock the sys_call_table_lock while editing and enable writing
    *   - store and hijack MY_CUSTOM_SYSCALL
    *	- store and hijack the original exit group
    *	- set the sys table back to read only state and do spin unlock
    *
    */
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
 ...
 */
 static void exit_function(void)
 {
 	...
 	*	- restore the original exit group
 	*	- set the sys table back to read only state and do spin unlock
 	*
 	*/
 	int i;
